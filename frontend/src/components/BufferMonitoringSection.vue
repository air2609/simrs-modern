...
onMounted(async () => {
  try {
    units.value = await request('/report/buffer/masters');
...
async function loadReport() {
  ...
  error.value = '';
  try {
    const unit = units.value.find((u) => u.unitId === unitId.value);
    const data = await request(`/report/buffer/report${qs({
  ...
  <div class="screen-page">
    ...
    <div class="card">
      <div class="filter-bar">
        ...
        <button class="small-button primary" type="button" :disabled="loading" @click="loadReport">👁️ LIHAT</button>
      </div>

      <div class="card-title">
        <span>DAFTAR STOK DIBAWAH BUFFER</span>
        <span v-if="searched" class="count-badge">{{ rows.length }} item</span>
      </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(row, index) in rows" :key="index">
              ...
              <td class="num">{{ row.buffer }}</td>
              <td>{{ row.satuan }}</td>
              <td class="num"><input class="order-input" type="number" min="0" /></td>
              <td class="num" :title="row.prCodes || ''">
                <span v-if="row.openOpp > 0" class="badge badge--open">{{ row.openOpp }}</span>
                <span v-else>{{ row.openOpp }}</span>
              </td>
            </tr>
            <tr v-if="!rows.length && !loading">
              <td colspan="8" class="empty-state">Pilih lokasi lalu tekan LIHAT.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
.card-title { display: flex; align-items: center; gap: 10px; margin: 12px 0 4px; font-weight: 800; color: #304b73; font-size: 14px; }
.count-badge { background: #eef3fa; color: #304b73; padding: 2px 10px; border-radius: 999px; font-size: 12px; }
...
.order-input { width: 70px; padding: 5px 7px; border: 1px solid #d1d9e6; border-radius: 6px; font-size: 13px; text-align: right; }

.badge { padding: 2px 10px; border-radius: 999px; font-size: 11px; font-weight: 700; }
.badge--open { background: #fef3e2; color: #b26a00; }
