...
const transaksiSetelahRetur = ref(0);

const drugTypes = [
  { value: '10', label: 'All' },
  { value: '1', label: '1. PSIKOTROPIKA' },
  { value: '2', label: '2. NARKOTIKA' },
  { value: '3', label: '3. GENERIK' },
  { value: '4', label: '4. PATEN' },
  { value: '5', label: '5. BPJS' }
];

const drugTypeLabel = computed(() => drugTypes.find((t) => t.value === drugType.value)?.label || 'All');
...
async function searchPatient() {
  ...
  error.value = '';
  try {
    patientResults.value = await request(`/cashier/rekap-obat/patients/ranap${qs(s)}`);
  } catch (requestError) {
...
async function selectPatient(result) {
  ...
  error.value = '';
  try {
    patient.value = await request(`/cashier/rekap-obat/patients/${encodeURIComponent(result.mrCode)}`);
...
async function loadRekap() {
  ...
  error.value = '';
  try {
    const data = await request(`/cashier/rekap-obat/rekap${qs({ registrationId: patient.value.registrationId, type: drugType.value })}`);
    rows.value = data.rows;
    totalTransaksi.value = data.totalTransaksi;
    totalRetur.value = data.totalRetur;
    transaksiSetelahRetur.value = data.transaksiSetelahRetur;
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
onMounted(() => {
  // tidak memuat data otomatis; pilih pasien dahulu
});
</script>
...
<template>
  <div class="screen-page">
    <div class="page-header">
      <h2>💊 FORM INFORMASI REKAP OBAT</h2>
      <p class="page-subtitle">Migrasi screen legacy SC0208 — rekapObat.zul (Rekap Obat Pasien Rawat Inap)</p>
    </div>
    ...
    <div class="card">
      ...
      <div class="section-title">LIST REKAP OBAT</div>
      <div class="filter-row">
        <span class="filter-label">FILTER TIPE OBAT :</span>
        <select v-model="drugType" @change="loadRekap">
          <option v-for="t in drugTypes" :key="t.value" :value="t.value">{{ t.label }}</option>
        </select>
      </div>

      <div v-if="loading" class="loading">Menghitung rekap obat...</div>
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th>NAMA OBAT</th>
              <th>TIPE</th>
              <th class="num">JML TRX</th>
              <th class="num">TOT TRX</th>
              <th class="num">JML RTR</th>
              <th class="num">TOT RTR</th>
          ...
          <tbody>
            <tr v-for="(row, index) in rows" :key="index">
              ...
              <td>{{ row.name }}</td>
              <td>{{ row.drugType }}</td>
              <td class="num">{{ row.jmlTrx }}</td>
              <td class="num">{{ fmtMoney(row.totTrx) }}</td>
              <td class="num">{{ row.jmlRetur }}</td>
              <td class="num">{{ fmtMoney(row.totRetur) }}</td>
              <td class="num strong">{{ fmtMoney(row.balance) }}</td>
            </tr>
            <tr v-if="!rows.length">
              <td colspan="8" class="empty-state">Pilih pasien rawat inap untuk melihat rekap obat.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="totals-line">
        <div class="field">
          ...
          <input :value="fmtMoney(totalTransaksi)" readonly />
        </div>
        <div class="field">
          <label>TOTAL RETUR</label>
          <input :value="fmtMoney(totalRetur)" readonly />
        </div>
        <div class="field">
          <label>TRANSAKSI SETELAH RETUR</label>
          <input :value="fmtMoney(transaksiSetelahRetur)" readonly class="highlight" />
        </div>
      </div>
...
.filter-row select { padding: 7px 9px; border: 1px solid #d1d9e6; border-radius: 6px; font-size: 13px; }
...
.totals-line { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 10px; }
