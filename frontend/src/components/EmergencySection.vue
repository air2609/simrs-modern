...
const patient = ref(null);       // EmergencyPatientDetailResponse
const note = ref(null);          // EmergencyNoteDetailResponse
const lines = ref([]);           // lines (editable saat transaksi)
const total = ref(0);
const editMode = ref(false);     // mode UBAH
const readOnly = ref(false);     // nota terkunci setelah simpan/load
const selectedLine = ref(null);  // index baris terpilih
...
onMounted(async () => {
  try {
    masters.value = await request('/emergency/masters');
  } catch (requestError) {
...
/** Alert / notifikasi dengan tombol OK. */
function showAlert(message, options = {}) {
  return new Promise((resolve) => {
    dialog.value = {
      visible: true, mode: 'alert',
...
/** Input dialog (mis. alasan pembatalan nota). */
...
async function searchPatient() {
  ...
  if (!s.mrCode && !s.name && !s.address && !s.birthDate) {
    showAlert('Salah satu field pencarian pasien harus diisi!');
    return;
  }
  loading.value = true;
  try {
    patientResults.value = await request(`/emergency/patients${qs(s)}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function selectPatient(result) {
  showModal.value = '';
  try {
    patient.value = await request(`/emergency/patients/${encodeURIComponent(result.mrCode)}`);
...
async function searchNote() {
  ...
  if (!s.noteNo && !s.name) {
    showAlert('Salah satu field pencarian nota harus diisi!');
    return;
  }
  loading.value = true;
  try {
    noteResults.value = await request(`/emergency/notes${qs({ noteNo: s.noteNo, patientName: s.name })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function loadNote(noteId) {
  loading.value = true;
  try {
    note.value = await request(`/emergency/notes/${noteId}`);
...
// ================= DOCTOR / TREATMENT / ITEM / MISC =================

async function searchDoctor() {
  const s = doctorSearch.value;
  loading.value = true;
  try {
    doctorResults.value = await request(`/emergency/doctors${qs(s)}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function searchTreatment() {
  ...
  if (!s.code && !s.name) {
    showAlert('Salah satu field harus diisi!');
    return;
  }
  loading.value = true;
  try {
    treatmentResults.value = await request(`/emergency/treatments${qs({ code: s.code, name: s.name })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function searchItem() {
  const s = itemSearch.value;
  loading.value = true;
  try {
    itemResults.value = await request(`/emergency/items${qs(s)}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
function newTransaction() {
  ...
  Object.assign(form.value, {
    ...
    age: '', address: '', patientTypeId: null, escortId: null, doctorId: null, doctorName: ''
  });
}
...
async function save() {
  ...
  const body = JSON.stringify({
    ...
    patientTypeId: form.value.patientTypeId,
  ...
  try {
    const result = editMode.value && note.value
      ? await request(`/emergency/notes/${note.value.noteId}`, { method: 'PUT', body })
      : await request('/emergency/notes', { method: 'POST', body });
...
async function validateNote() {
  ...
  error.value = '';
  try {
    const result = await request(`/emergency/notes/${note.value.noteId}/validate`, { method: 'POST' });
...
async function cancelNote() {
  ...
  error.value = '';
  try {
    const result = await request(`/emergency/notes/${note.value.noteId}/cancel${qs({ reason })}`, { method: 'POST' });
...
function printNote() {
  ...
  window.open(`${props.apiBaseUrl}/emergency/notes/${note.value.noteId}/print`, '_blank');
}
...
async function loadHistory() {
  ...
  error.value = '';
  try {
    const data = await request(`/emergency/history${qs({ mrCode: form.value.mrCode || patient.value.mrCode, mode: historyMode.value })}`);
...
const historyLineCount = computed(() => {
  if (!selectedHistoryNote.value) return 0;
  return selectedHistoryNote.value.lines.length;
});
...
<template>
  <div class="screen-page">
    <div class="page-header">
      <h2>🚑 TRANSAKSI UGD</h2>
      <p class="page-subtitle">Migrasi screen legacy SC0061 — emergency.zul (FORM INPUT TRANSAKSI UGD)</p>
    </div>
    ...
    <div class="card">
      ...
      <div v-if="activeTab === 'transaksi'">
        ...
        <div class="patient-grid">
          <div class="field">
            ...
            <input :value="masters ? '1. ' + masters.unitName : '1. UGD'" readonly />
          </div>
          <div class="field">
            <label>NO. NOTA</label>
            <div class="input-row">
          ...
          <div class="field">
            ...
            <div class="input-row">
              ...
              <button class="mini primary" type="button" :disabled="readOnly" @click="showModal = 'doctor'">CARI DOKTER</button>
            </div>
          </div>
          <div class="field">
        ...
        <div class="section-title">DATA TINDAKAN UGD</div>
          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(line, index) in lines" :key="index">
                <td>
                  <input type="radio" name="lineSelect" :value="index" v-model="selectedLine" />
                </td>
                <td>{{ line.code }}</td>
                <td>{{ line.name }}</td>
                <td class="num">
      ...
      <div v-else>
        <div class="section-title">
          <span>{{ patient ? `${patient.patientName} (${patient.mrCode})` : (form.patientName ? `${form.patientName} (${form.mrCode})` : 'HISTORY TRANSAKSI') }}</span>
        </div>
            ...
            <div class="table-wrap">
              <table class="table">
                ...
                <tbody>
                  ...
                  <tr v-if="!historyLineCount">
                    <td colspan="4" class="empty-state">Pilih nota di sebelah kiri.</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div class="grand-total">
          GRAND TOTAL : <strong>{{ fmtMoney(historyGrandTotal) }}</strong>
        </div>
      </div>
    </div>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="table-wrap modal-list">
            <table class="table">
              <thead>
                <tr><th>NO. MR</th><th>NAMA</th><th>TGL</th><th>ALAMAT</th></tr>
              </thead>
              <tbody>
      ...
      <div class="modal">
        ...
        <div class="modal-body">
          ...
          <div class="table-wrap modal-list">
            <table class="table">
              <thead>
                <tr><th>NO. NOTA</th><th>NAMA</th><th>STATUS NOTA</th></tr>
              </thead>
              <tbody>
...
.status-label { font-weight: 700; color: #a32943; font-size: 13px; }
.status-value { font-weight: 700; color: #a32943; font-size: 13px; }
