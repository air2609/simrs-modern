...
onMounted(async () => {
  try {
    masters.value = await request('/physiotherapy/masters');
...
async function searchPatient() {
  ...
  error.value = '';
  try {
    patientResults.value = await request(`/physiotherapy/patients/registered${qs(s)}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function selectPatient(result) {
  showModal.value = '';
  try {
    patient.value = await request(`/physiotherapy/patients/${encodeURIComponent(result.mrCode)}`);
...
async function searchNote() {
  ...
  error.value = '';
  try {
    noteResults.value = await request(`/physiotherapy/notes${qs({ unitId: form.value.unitId, noteNo: s.noteNo, patientName: s.name })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function loadNote(noteId) {
  loading.value = true;
  try {
    note.value = await request(`/physiotherapy/notes/${noteId}`);
...
async function searchDoctor() {
  ...
  error.value = '';
  try {
    doctorResults.value = await request(`/physiotherapy/doctors${qs(s)}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function searchTreatment() {
  ...
  error.value = '';
  try {
    treatmentResults.value = await request(`/physiotherapy/treatments${qs({ code: s.code, name: s.name })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function searchItem() {
  ...
  error.value = '';
  try {
    itemResults.value = await request(`/physiotherapy/items${qs({ warehouseId: unit.warehouseId, code: itemSearch.value.code, name: itemSearch.value.name })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function save() {
  ...
  try {
    const result = editMode.value && note.value
      ? await request(`/physiotherapy/notes/${note.value.noteId}`, { method: 'PUT', body })
      : await request('/physiotherapy/notes', { method: 'POST', body });
...
async function validateNote() {
  ...
  error.value = '';
  try {
    const result = await request(`/physiotherapy/notes/${note.value.noteId}/validate`, { method: 'POST' });
...
async function cancelNote() {
  ...
  error.value = '';
  try {
    const result = await request(`/physiotherapy/notes/${note.value.noteId}/cancel${qs({ reason })}`, { method: 'POST' });
...
function printNote() {
  ...
  window.open(`${props.apiBaseUrl}/physiotherapy/notes/${note.value.noteId}/print`, '_blank');
}
...
async function loadHistory() {
  ...
  error.value = '';
  try {
    const data = await request(`/physiotherapy/history${qs({ mrCode: form.value.mrCode || patient.value.mrCode, mode: historyMode.value })}`);
...
<template>
  <div class="screen-page">
    <div class="page-header">
      <h2>💪 TRANSAKSI FISIOTERAPI</h2>
      <p class="page-subtitle">Migrasi screen legacy SC0141 — physiotherapy.zul (FORM INPUT TRANSAKSI FISIOTERAPI)</p>
    </div>
    ...
    <div class="card">
      ...
      <div v-if="activeTab === 'transaksi'">
        ...
        <div class="section-title">DATA TINDAKAN FISIOTERAPI</div>
