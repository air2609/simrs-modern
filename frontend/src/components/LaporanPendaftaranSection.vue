...
const error = ref('');
const units = ref([]);
const unitId = ref(0); // 0 = SEMUA

const fromDate = ref(todayIso());
const toDate = ref(todayIso());

const rows = ref([]);
const totals = ref({ lakiLaki: 0, perempuan: 0, lama: 0, baru: 0, keseluruhan: 0 });
...
onMounted(async () => {
  try {
    units.value = await request('/report/laporan-pendaftaran/masters');
  } catch (requestError) {
...
async function loadReport() {
  ...
  error.value = '';
  try {
    const data = await request(`/report/laporan-pendaftaran/report${qs({
      from: fromDate.value,
      to: toDate.value,
    ...
    totals.value = {
      lakiLaki: data.totalLakiLaki,
      perempuan: data.totalPerempuan,
      lama: data.totalLama,
      baru: data.totalBaru,
      keseluruhan: data.totalKeseluruhan
    };
    searched.value = true;
  } catch (requestError) {
    error.value = requestError.message;
...
function printReport() {
  if (!searched.value) {
    showAlert('Tekan LIHAT DATA terlebih dahulu!');
    return;
  }
  window.open(`${props.apiBaseUrl}/report/laporan-pendaftaran/print${qs({
    from: fromDate.value,
    to: toDate.value,
    unitId: unitId.value || undefined
  })}`, '_blank');
...
function exportXls() {
  ...
  const headerRow = `<Row>${['TANGGAL', 'UNIT', 'LAKI-LAKI', 'PEREMPUAN', 'LAMA', 'BARU', 'TOTAL'].map((h) => cell(h)).join('')}</Row>`;
  const dataRows = rows.value.map((r) =>
    `<Row>${cell(r.tanggal)}${cell(r.unit)}${cell(r.lakiLaki, 'Number')}${cell(r.perempuan, 'Number')}${cell(r.lama, 'Number')}${cell(r.baru, 'Number')}${cell(r.total, 'Number')}</Row>`
  ).join('\n');
  const totalRow = `<Row>${cell('')}${cell('T O T A L')}${cell(totals.lakiLaki, 'Number')}${cell(totals.perempuan, 'Number')}${cell(totals.lama, 'Number')}${cell(totals.baru, 'Number')}${cell(totals.keseluruhan, 'Number')}</Row>`;

  const xml = `<?xml version="1.0"?>
<?mso-application progid="Excel.Sheet"?>
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
 <Worksheet ss:Name="LaporanPendaftaran">
  <Table>
   ${headerRow}
   ${dataRows}
   ${totalRow}
  ...
  link.href = url;
  link.download = 'laporan_pendaftaran.xls';
...
<template>
  <div class="screen-page">
    <div class="page-header">
      ...
      <p class="page-subtitle">Migrasi screen legacy RPT0010 — laporanPendaftaran.zul</p>
    </div>
    ...
    <div class="card">
      <div class="filter-bar">
        ...
        <button class="small-button primary" type="button" :disabled="loading" @click="loadReport">👁️ LIHAT DATA</button>
      </div>
        ...
        <table class="table">
          <thead>
            <tr>
              <th>TANGGAL</th>
              <th>UNIT</th>
              <th class="num">LAKI-LAKI</th>
              <th class="num">PEREMPUAN</th>
              <th class="num">LAMA</th>
              <th class="num">BARU</th>
          ...
          <tbody>
            <tr v-for="(row, index) in rows" :key="index">
              ...
              <td class="num">{{ row.lakiLaki }}</td>
              <td class="num">{{ row.perempuan }}</td>
              <td class="num">{{ row.lama }}</td>
              <td class="num">{{ row.baru }}</td>
              <td class="num strong">{{ row.total }}</td>
            </tr>
            <tr v-if="searched" class="total-row">
              <td></td>
              <td class="strong">T O T A L</td>
              <td class="num strong">{{ totals.lakiLaki }}</td>
              <td class="num strong">{{ totals.perempuan }}</td>
              <td class="num strong">{{ totals.lama }}</td>
              <td class="num strong">{{ totals.baru }}</td>
              <td class="num strong">{{ totals.keseluruhan }}</td>
            </tr>
            <tr v-if="!rows.length && !loading">
              <td colspan="7" class="empty-state">Pilih rentang tanggal lalu tekan LIHAT DATA.</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="action-bar">
        <button class="small-button" type="button" :disabled="!searched" @click="exportXls">📥 EXPORT TO XLS</button>
        <button class="small-button" type="button" :disabled="!searched" @click="printReport">🖨️ CETAK</button>
...
.filter-bar input[type="date"], .filter-bar select { padding: 7px 9px; border: 1px solid #d1d9e6; border-radius: 6px; font-size: 13px; }
