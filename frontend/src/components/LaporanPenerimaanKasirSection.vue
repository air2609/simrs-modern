...
const transDate = ref(todayIso());
const shift = ref('2'); // 2 PAGI (default), 1 SORE, 3 MALAM, A ALL
...
const shiftOptions = [
  ...
  { value: 'A', label: 'ALL SHIFT' }
];

const isTotalRow = (row) => row.noKwitansi === 'TOTAL';
...
async function loadReport() {
  if (!transDate.value) {
    await showAlert('Isi tanggal terlebih dahulu');
    return;
  }
  loading.value = true;
  error.value = '';
  try {
    const data = await request(`/report/penerimaan-kasir/report${qs({
...
<template>
  <div class="screen-page">
    <div class="page-header">
      <h2>💵 LAPORAN REKAP PENERIMAAN KASIR</h2>
      <p class="page-subtitle">Migrasi screen legacy RPT0012 — laporanKasir.zul</p>
    </div>
    ...
    <div class="card">
      <div class="filter-bar">
        ...
        <input v-model="transDate" type="date" />
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(row, index) in rows" :key="index" :class="{ 'total-row': isTotalRow(row) }">
              ...
              <td :class="{ strong: isTotalRow(row) }">{{ row.noKwitansi }}</td>
              <td class="num strong">{{ fmtMoney(row.tunai) }}</td>
              <td class="num strong">{{ fmtMoney(row.nonTunai) }}</td>
              <td>{{ row.kasir }}</td>
            </tr>
            <tr v-if="!rows.length && !loading">
              <td colspan="5" class="empty-state">Pilih tanggal &amp; shift lalu tekan LIHAT LAPORAN.</td>
            </tr>
          </tbody>
        </table>
      </div>
