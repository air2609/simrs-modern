...
async function loadReport() {
  ...
  error.value = '';
  try {
    const data = await request(`/report/earning${qs({
      tipe: tipe.value,
      from: fromDate.value,
      to: toDate.value,
...
function exportXls() {
  ...
  let dataRows;
  if (tipe.value === 'PD') {
    titleRow = `LAPORAN PENDAPATAN ${title} - ${title2} TIPE PASIEN : ${pasLabel}`;
    ...
    titleRow = `LAPORAN SUMBANGSIH PENJUALAN OBAT ${title} - ${title2} TIPE PASIEN : ${pasLabel}`;
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="EarningReport">
  <Table>
      ...
      <div v-else class="table-wrap">
        ...
        <table v-if="tipe === 'PD'" class="table">
          ...
          <tbody>
            ...
            <tr v-if="!pagedRows.length && !loading">
              <td colspan="9" class="empty-state">Pilih periode &amp; tipe pasien lalu tekan VIEW.</td>
            </tr>
          </tbody>
        </table>
        <!-- OBAT -->
        <table v-else-if="tipe === 'OBAT'" class="table">
          ...
          <tbody>
            ...
            <tr v-if="!pagedRows.length && !loading">
              <td colspan="4" class="empty-state">Pilih periode &amp; tipe pasien lalu tekan VIEW.</td>
            </tr>
          </tbody>
        </table>
        <!-- ALL -->
        <table v-else class="table">
          ...
          <tbody>
            ...
            <tr v-if="!allRows.length && !loading">
              <td colspan="3" class="empty-state">Pilih periode &amp; tipe pasien lalu tekan VIEW.</td>
            </tr>
          </tbody>
        </table>
      </div>
