...
async function loadTreatmentClasses() {
  rows.value = await request('/master/treatment-class');
}
...
async function doSave() {
  ...
  saving.value = true;
  try {
    await request('/master/treatment-class/save', {
      method: 'POST',
...
async function doDelete() {
  error.value = '';
  if (!selectedId.value) {
    error.value = 'Pilih data kelas tarif terlebih dahulu.';
    return;
  }
  if (!window.confirm('Yakin ingin menghapus data kelas tarif ini?')) {
    return;
  }
  try {
    await request(`/master/treatment-class/delete?id=${selectedId.value}`, {
      method: 'DELETE'
    });
    resetForm();
    await loadTreatmentClasses();
...
<template>
  <div class="screen-page">
    ...
    <div class="page-header">
      <div>
        <h2>🏷️ Kelas Tarif</h2>
        <p class="page-subtitle">Migrasi form legacy SCM0021 — master kelas tarif perawatan</p>
      </div>
    ...
    <div v-if="loading" class="loading">Memuat data kelas tarif...</div>

    <template v-else>
      <!-- Form -->
      <div class="card">
        <h3 class="card-title">FORM KELAS TARIF</h3>
        <div class="form-grid">
          <div class="field">
            <label for="tclass-code">KODE</label>
            <input
              id="tclass-code"
              v-model="form.code"
              type="text"
              maxlength="15"
              placeholder="Kode kelas tarif"
              @keyup.enter="doSave"
            />
          </div>
          <div class="field">
            <label for="tclass-name">NAMA</label>
            <input
              id="tclass-name"
              v-model="form.description"
              type="text"
              maxlength="50"
              placeholder="Nama kelas tarif"
              @keyup.enter="doSave"
            />
          </div>
        </div>
      ...
      <!-- List -->
      <div class="card">
        <h3 class="card-title">DATA KELAS TARIF</h3>
        <div class="table-wrap">
          <table class="table">
            ...
            <tbody>
              ...
              <tr v-if="!rows.length">
                <td colspan="2" class="empty-state">Tidak ada data kelas tarif.</td>
              </tr>
            </tbody>
          </table>
        </div>
