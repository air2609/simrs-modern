...
// ---- inventory pasien ----
const invModal = ref(false);
const invData = ref(null);
const invQty = ref({});
const invHistorySelected = ref([]);
...
onMounted(async () => {
  try {
    masters.value = await request('/ward/masters');
...
async function searchPatient() {
  ...
  error.value = '';
  try {
    patientResults.value = await request(`/ward/patients/ranap${qs(s)}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function selectPatient(result) {
  showModal.value = '';
  try {
    patient.value = await request(`/ward/patients/${encodeURIComponent(result.mrCode)}`);
...
function applyPatient(p) {
  ...
  form.value.patientTypeName = p.patientTypeName || '';
  form.value.doctorId = p.doctorId ?? null;
  form.value.doctorName = p.doctorName || '';
  form.value.treatmentClass = p.treatmentClass || '';
...
async function searchNote() {
  ...
  error.value = '';
  try {
    noteResults.value = await request(`/ward/notes${qs({ unitId: form.value.unitId, noteNo: s.noteNo, patientName: s.name })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function loadNote(noteId) {
  loading.value = true;
  try {
    note.value = await request(`/ward/notes/${noteId}`);
    ...
    form.value.patientTypeName = note.value.patientTypeName || '';
    form.value.doctorId = note.value.doctorId ?? null;
    form.value.doctorName = note.value.doctorName || '';
    form.value.treatmentClass = note.value.treatmentClass || '';
...
async function setMainDoctor(result) {
  try {
    await request(`/ward/registrations/${patient.value.registrationId}/doctor${qs({ doctorId: result.staffId })}`, { method: 'POST' });
    form.value.doctorId = result.staffId;
    form.value.doctorName = `${result.code}-${result.name}`;
    showToast('DOKTER UTAMA BERHASIL DIUBAH', 'success');
  } catch (requestError) {
    error.value = requestError.message;
  }
  showModal.value = '';
...
async function searchTreatment() {
  ...
  error.value = '';
  try {
    treatmentResults.value = await request(`/ward/treatments${qs({ code: s.code, name: s.name, tariffClass: form.value.treatmentClass })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
async function searchItem() {
  ...
  error.value = '';
  try {
    itemResults.value = await request(`/ward/items${qs({ warehouseId: unit.warehouseId, code: itemSearch.value.code, name: itemSearch.value.name })}`);
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
    loading.value = false;
...
function newTransaction() {
  ...
  Object.assign(form.value, {
    ...
    age: '', address: '', patientTypeName: '', doctorId: null, doctorName: '',
    treatmentClass: '', hall: '', bed: ''
  });
}
...
async function save() {
  ...
  try {
    const result = editMode.value && note.value
      ? await request(`/ward/notes/${note.value.noteId}`, { method: 'PUT', body })
      : await request('/ward/notes', { method: 'POST', body });
...
async function validateNote() {
  ...
  error.value = '';
  try {
    const result = await request(`/ward/notes/${note.value.noteId}/validate`, { method: 'POST' });
...
async function cancelNote() {
  ...
  error.value = '';
  try {
    const result = await request(`/ward/notes/${note.value.noteId}/cancel${qs({ reason })}`, { method: 'POST' });
...
function printNote() {
  ...
  window.open(`${props.apiBaseUrl}/ward/notes/${note.value.noteId}/print`, '_blank');
}
...
async function openInventory() {
  ...
  error.value = '';
  try {
    invData.value = await request(`/ward/patient-inventory${qs({ registrationId: patient.value.registrationId })}`);
    invQty.value = {};
    invHistorySelected.value = [];
    invModal.value = true;
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
...
async function saveInventory() {
  const linesToSave = invData.value.items
    .filter((item) => invQty.value[item.itemId])
    .map((item) => ({ itemId: item.itemId, qtyOut: invQty.value[item.itemId] }));
  if (!linesToSave.length) {
    await showAlert('TIDAK ADA PEMAKAIAN YANG DIISI!');
    return;
  }
  for (const line of linesToSave) {
    if (line.qtyOut < 0) {
      await showAlert('INPUT NEGATIF TIDAK DIPERBOLEHKAN!');
      return;
    }
    const item = invData.value.items.find((i) => i.itemId === line.itemId);
    if (line.qtyOut > item.sisa) {
      await showAlert(`PEMAKAIAN ${item.name} MELEBIHI SISA (${item.sisa})!`);
      return;
    }
  }
  loading.value = true;
  error.value = '';
  try {
    await request('/ward/patient-inventory', {
      method: 'POST',
      body: JSON.stringify({ registrationId: invData.value.registrationId, lines: linesToSave })
    });
    showToast('Inventory pasien berhasil disimpan.', 'success');
    invData.value = await request(`/ward/patient-inventory${qs({ registrationId: invData.value.registrationId })}`);
    invQty.value = {};
  } catch (requestError) {
    error.value = requestError.message;
  } finally {
...
async function deleteInventoryHistory() {
  if (!invHistorySelected.value.length) {
    await showAlert('PILIH RIWAYAT INVENTORY TERLEBIH DAHULU!');
    return;
  }
  const ok = await showConfirm(`Hapus ${invHistorySelected.value.length} riwayat inventory?`);
  if (!ok) return;
  loading.value = true;
  error.value = '';
  try {
    for (const piId of invHistorySelected.value) {
      await request(`/ward/patient-inventory/${piId}/delete`, { method: 'POST' });
    }
    showToast('Riwayat inventory berhasil dihapus.', 'success');
    invData.value = await request(`/ward/patient-inventory${qs({ registrationId: invData.value.registrationId })}`);
    invHistorySelected.value = [];
  } catch (requestError) {
    error.value = requestError.message;
...
function toggleInvHistory(piId) {
  const index = invHistorySelected.value.indexOf(piId);
  if (index >= 0) invHistorySelected.value.splice(index, 1);
  else invHistorySelected.value.push(piId);
}

function itemName(itemId) {
  const found = (invData.value?.items || []).find((i) => i.itemId === itemId);
...
async function loadHistory() {
  ...
  error.value = '';
  try {
    const data = await request(`/ward/history${qs({ mrCode: form.value.mrCode || patient.value.mrCode, mode: historyMode.value })}`);
...
<template>
  <div class="screen-page">
    <div class="page-header">
      <h2>🛏️ TRANSAKSI BANGSAL</h2>
      <p class="page-subtitle">Migrasi screen legacy SC0031 — ward.zul (FORM INPUT TRANSAKSI BANGSAL)</p>
    </div>
    ...
    <div class="card">
      ...
      <div v-if="activeTab === 'transaksi'">
        ...
        <div class="patient-grid">
          ...
          <div class="field">
            ...
            <div class="input-row">
              ...
              <button class="mini primary" type="button" @click="showModal = 'doctor'">CARI DOKTER</button>
            </div>
          </div>
          <div class="field">
            <label>KELAS TARIF</label>
            <input v-model="form.treatmentClass" readonly />
          </div>
          <div class="field">
            <label>RUANGAN</label>
        ...
        <div class="section-title">DATA TINDAKAN BANGSAL</div>
        ...
        <div class="action-bar">
          ...
          <button class="small-button primary" type="button" @click="openInventory">🧺 INVENTORY PASIEN</button>
    ...
    <!-- ==================== MODAL: CARI DOKTER ==================== -->
    <div v-if="showModal === 'doctor'" class="modal-overlay" @click.self="showModal = ''">
      <div class="modal">
        <div class="modal-header">CARI DOKTER</div>
        <div class="modal-body">
          ...
          <div class="table-wrap modal-list">
            <table class="table">
              ...
              <tbody>
                <tr v-for="r in doctorResults" :key="r.staffId" @click="setMainDoctor(r)">
    ...
    <!-- ==================== MODAL: INVENTORY PASIEN ==================== -->
    <div v-if="invModal" class="modal-overlay" @click.self="invModal = false">
      <div class="modal modal--wide">
        <div class="modal-header">
          INVENTORY PASIEN — {{ invData ? `${invData.patientName} (${invData.mrCode})` : '' }}
        </div>
        <div class="modal-body">
          <div class="section-title">HISTORY INVENTORY PASIEN</div>
          <div class="table-wrap modal-list">
            <table class="table">
              <thead><tr><th>NAMA ITEM</th><th>TANGGAL</th><th class="num">JLH DATANG</th><th class="num">JLH DIPAKAI</th><th class="num">SISA</th><th></th></tr></thead>
              <tbody>
                <tr v-for="h in (invData?.history || [])" :key="h.piId">
                  <td>{{ itemName(h.itemId) }}</td>
                  <td>{{ h.date }}</td>
                  <td class="num">{{ h.qtyIn }}</td>
                  <td class="num">{{ h.qtyOut }}</td>
                  <td class="num">{{ h.sisa }}</td>
                  <td><input type="checkbox" :checked="invHistorySelected.includes(h.piId)" @change="toggleInvHistory(h.piId)" /></td>
                </tr>
                <tr v-if="!(invData?.history || []).length">
                  <td colspan="6" class="empty-state">Belum ada riwayat inventory.</td>
                </tr>
              </tbody>
            </table>
          </div>
          <div class="action-bar">
            <button class="small-button danger" type="button" @click="deleteInventoryHistory">🗑️ HAPUS</button>
          </div>

          <div class="section-title">LIST INVENTORY PASIEN</div>
          <div class="table-wrap modal-list">
            <table class="table">
              <thead><tr><th>KODE</th><th>NAMA</th><th class="num">TOTAL</th><th class="num">TERPAKAI</th><th class="num">SISA</th><th>SATUAN</th><th class="num">QTY KELUAR</th></tr></thead>
              <tbody>
                <tr v-for="item in (invData?.items || [])" :key="item.itemId">
                  <td class="strong">{{ item.code }}</td>
                  <td>{{ item.name }}</td>
                  <td class="num">{{ item.totalIn }}</td>
                  <td class="num">{{ item.totalOut }}</td>
                  <td class="num">{{ item.sisa }}</td>
                  <td>{{ item.unit }}</td>
                  <td class="num"><input v-model.number="invQty[item.itemId]" class="num-input" type="number" min="0" /></td>
                </tr>
                <tr v-if="!(invData?.items || []).length">
                  <td colspan="7" class="empty-state">Belum ada item inventory.</td>
                </tr>
              </tbody>
            </table>
          </div>
          <div class="action-bar">
            <button class="small-button primary" type="button" :disabled="loading" @click="saveInventory">💾 SIMPAN</button>
          </div>
        </div>
        <div class="modal-footer">
          <button class="small-button" type="button" @click="invModal = false">TUTUP</button>
        </div>
      </div>
    </div>
