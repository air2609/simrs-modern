...
onMounted(async () => {
  try {
    units.value = await request('/report/persediaan-obat/masters');
...
async function loadReport() {
  ...
  error.value = '';
  try {
    const data = await request(`/report/persediaan-obat/report${qs({ unitId: unitId.value })}`);
    rows.value = data.rows;
    unitName.value = data.unitName;
    periodeLabel.value = data.periodeLabel;
    currentPage.value = 1;
    searched.value = true;
  } catch (requestError) {
    error.value = requestError.message;
...
function printReport() {
  ...
  window.open(`${props.apiBaseUrl}/report/persediaan-obat/print${qs({
    unitId: unitId.value
  })}`, '_blank');
}

// EXPORT TO XLS (SpreadsheetML) — struktur mengikuti export legacy:
// judul RS+unit, judul laporan, PER tanggal, header, data
function exportXls() {
  if (!searched.value) {
    showAlert('Tekan LIHAT LAPORAN terlebih dahulu!');
    return;
  ...
  const perLabel = `PER ${pad(now.getDate())}-${months[now.getMonth()]}-${now.getFullYear()} ${pad(now.getHours())}:${pad(now.getMinutes())}`;

  const titleRow1 = `<Row>${cell(`RUMAH SAKIT TIARA SELLA UNIT ${unitName.value}`)}</Row>`;
  const titleRow2 = `<Row>${cell('LAPORAN PERSEDIAAN OBAT dan BAHAN MEDIS')}</Row>`;
  const titleRow3 = `<Row>${cell(perLabel)}</Row>`;
  const blankRow = '<Row></Row>';
  const headerRow = `<Row>${['No', 'Kode Obat', 'Nama Obat', 'Harga', 'Jumlah', 'Satuan'].map((h) => cell(h)).join('')}</Row>`;
  const dataRows = rows.value.map((r) =>
    `<Row>${cell(r.nomor ?? '', 'Number')}${cell(r.kodeObat ?? '')}${cell(r.namaObat ?? '')}${cell(fmtMoney(r.hargaStandar), 'Number')}${cell(fmtMoney(r.jumlah), 'Number')}${cell(r.satuan ?? '')}</Row>`
  ).join('\n');
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="PersediaanObat">
  <Table>
   ${titleRow1}
   ${titleRow2}
   ${titleRow3}
  ...
  link.href = url;
  link.download = 'laporan_persediaan_obat.xls';
  ...
  <div class="screen-page">
    ...
    <div class="card">
      ...
      <div v-if="searched" class="periode-line">
        <span class="periode-label">UNIT</span>
        <span class="periode-value">{{ unitName }}</span>
        <span class="periode-label">PERIODE</span>
        <span class="periode-value">{{ periodeLabel }}</span>
      </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(row, index) in pagedRows" :key="index">
              <td>{{ row.nomor ?? '' }}</td>
              <td class="strong">{{ row.kodeObat ?? '' }}</td>
              <td>{{ row.namaObat ?? '' }}</td>
              <td class="num">{{ fmtMoney(row.hargaStandar) }}</td>
              <td class="num">{{ fmtMoney(row.jumlah) }}</td>
              <td>{{ row.satuan ?? '' }}</td>
            </tr>
            <tr v-if="!pagedRows.length && !loading">
              <td colspan="6" class="empty-state">Pilih lokasi lalu tekan LIHAT LAPORAN.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
.filter-bar select { padding: 7px 9px; border: 1px solid #d1d9e6; border-radius: 6px; font-size: 13px; }
...
.table-wrap { overflow: auto; margin: 10px 0; max-height: 480px; }
