...
const tipePasien = ref('BPJS'); // default sesuai legacy (selected="true")
...
async function loadReport() {
  ...
  error.value = '';
  try {
    const data = await request(`/report/rekap-tindakan/report${qs({
      from: fromDate.value,
      to: toDate.value,
      tipePasien: tipePasien.value
    })}`);
    rows.value = data.rows;
    totalQty.value = data.totalQty;
    totalNominal.value = data.totalNominal;
    searched.value = true;
  } catch (requestError) {
...
// EXPORT TO XLS (SpreadsheetML), mengikuti judul export legacy
// "LAPORAN REKAP TINDAKAN PERIODE {dd/MM/yyyy} - {dd/MM/yyyy} TIPE PASIEN {label}"
function exportXls() {
  if (!searched.value) {
  ...
  const title = `LAPORAN REKAP TINDAKAN PERIODE ${fmtDisplayDate(fromDate.value)} - ${fmtDisplayDate(toDate.value)} TIPE PASIEN ${tipePasien.value}`;
  const titleRow = `<Row>${cell(title)}</Row>`;
  const headerRow = `<Row>${['KODE', 'NAMA TINDAKAN', 'QTY', 'TOTAL'].map((h) => cell(h)).join('')}</Row>`;
  const dataRows = rows.value.map((r) =>
    `<Row>${cell(r.kode)}${cell(r.namaTindakan)}${cell(fmtNumber(r.qty), 'Number')}${cell(fmtNumber(r.total), 'Number')}</Row>`
  ).join('\n');
  const totalRow = `<Row>${cell('')}${cell('T O T A L')}${cell(fmtNumber(totalQty.value), 'Number')}${cell(fmtNumber(totalNominal.value), 'Number')}</Row>`;

  const xml = `<?xml version="1.0"?>
<?mso-application progid="Excel.Sheet"?>
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
 <Worksheet ss:Name="RekapTindakan">
  <Table>
   ${titleRow}
   ${headerRow}
   ${dataRows}
  ...
  link.href = url;
  link.download = 'rekap_tindakan.xls';
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(row, index) in rows" :key="index">
              <td class="strong">{{ row.kode }}</td>
              <td>{{ row.namaTindakan }}</td>
              <td class="num">{{ fmtNumber(row.qty) }}</td>
              <td class="num strong">{{ fmtNumber(row.total) }}</td>
            </tr>
            <tr v-if="searched" class="total-row">
              <td></td>
              <td class="strong">T O T A L</td>
              <td class="num strong">{{ fmtNumber(totalQty) }}</td>
              <td class="num strong">{{ fmtNumber(totalNominal) }}</td>
            </tr>
            <tr v-if="!rows.length && !loading">
              <td colspan="4" class="empty-state">Pilih rentang tanggal lalu tekan LIHAT LAPORAN.</td>
            </tr>
          </tbody>
        </table>
      </div>
