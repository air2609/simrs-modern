...
async function loadIcds() {
  rows.value = await request('/master/icd');
  currentPage.value = 1;
}
...
async function doSave() {
  ...
  if (!form.value.code || !form.value.name) {
    error.value = 'Kode ICD dan Nama Penyakit harus diisi.';
    return;
  }
  saving.value = true;
  try {
    await request('/master/icd/save', {
      method: 'POST',
...
async function doDelete() {
  error.value = '';
  if (!selectedId.value) {
    error.value = 'Pilih data ICD terlebih dahulu.';
    return;
  }
  if (!window.confirm('Yakin ingin menghapus data ICD ini?')) {
    return;
  }
  try {
    await request(`/master/icd/delete?id=${selectedId.value}`, {
      method: 'DELETE'
    });
    resetForm();
    await loadIcds();
...
<template>
  <div class="screen-page">
    ...
    <div v-if="loading" class="loading">Memuat data ICD...</div>

    <template v-else>
      <!-- Form -->
      <div class="card">
        <h3 class="card-title">FORM ICD</h3>
        <div class="form-grid">
          <div class="field">
            <label for="icd-code">KODE ICD</label>
            <input
              id="icd-code"
              v-model="form.code"
              type="text"
          ...
          <div class="field">
            <label for="icd-name">NAMA PENYAKIT</label>
            <input
              id="icd-name"
              v-model="form.name"
              type="text"
            ...
            <tbody>
              ...
              <tr v-if="!filteredRows.length">
                <td colspan="2" class="empty-state">
                  {{ rows.length ? 'Tidak ada data yang cocok dengan pencarian.' : 'Tidak ada data ICD.' }}
                </td>
              </tr>
            </tbody>
          </table>
