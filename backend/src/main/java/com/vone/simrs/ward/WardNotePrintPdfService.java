...
/**
 * Generator PDF NOTA BANGSAL untuk tombol CETAK screen SC0031. Migrasi dari
 * legacy {@code WardTransactionController.cetak()} + {@code NoteReport}.
 */
@Service
public class WardNotePrintPdfService {

    private static final DecimalFormat MONEY = new DecimalFormat("#,##0.###");

    private final WardService wardService;

    public WardNotePrintPdfService(WardService wardService) {
        this.wardService = wardService;
    }

    public byte[] generateNotePdf(Integer noteId) throws Exception {
        WardNoteDetailResponse note = wardService.getNoteDetail(noteId);
        ...
        document.add(new Paragraph("NOTA TRANSAKSI BANGSAL", new Font(Font.HELVETICA, 11, Font.BOLD)));
        ...
        info.addCell(labelCell(new Phrase("KELAS TARIF", labelFont)));
        info.addCell(valueCell(new Phrase(nvl(note.getTreatmentClass()), normal)));
