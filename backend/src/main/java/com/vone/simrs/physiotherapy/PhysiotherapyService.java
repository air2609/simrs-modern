...
/**
 * Service untuk screen SC0141 (TRANSAKSI FISIOTERAPI / physiotherapy.zul).
 *
 * <p>
 * Migrasi dari legacy {@code FisioterapiController} +
 * {@code RadiologyManagerImpl.getRegistrationFisio()/getNoteDetailFisio()} +
 * {@code PolyclinicDAO.saveTransaction()} + {@code NoteDAO.save()} (journal).
 * Mirip transaksi bangsal, namun pasien bisa rawat jalan maupun rawat inap
 * (nomor nota menyesuaikan: "J-" untuk rajal, "I-" untuk ranap).
 */
@Service
public class PhysiotherapyService {
    ...
    public PhysiotherapyService(JdbcTemplate jdbcTemplate, LegacyAuthService legacyAuthService) {
    ...
    public PhysiotherapyMastersResponse getMasters(String username) {
        ...
        return new PhysiotherapyMastersResponse(units, patientTypes, escorts);
    }

    // ------------------------------------------------------------------ pasien

    /**
     * Cari pasien terdaftar (memiliki registrasi aktif). Migrasi dari
     * {@code MsPatientDAO.searchPatientRegistered(...)}.
     */
    ...
    /**
     * Detail pasien + registrasi terakhir + bed (jika rawat inap). Migrasi dari
     * {@code RadiologyManagerImpl.getRegistrationFisio()}.
     */
    ...
    /** Simpan nota fisioterapi (nomor nota mengikuti tipe registrasi rajal/ranap). */
    @Transactional
    ...
    /** Ubah nota fisioterapi: restore stok, hapus baris, simpan ulang. */
    @Transactional
    ...
    /** Validasi nota + journal. Migrasi dari {@code NoteDAO.save()}. */
    @Transactional
    ...
    /** Riwayat transaksi pasien (tab HISTORY). */
    ...
    /** Nomor nota: "I-" untuk rawat inap, "J-" untuk rawat jalan. */
