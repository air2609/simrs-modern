...
/**
 * Generator PDF NOTA FISIOTERAPI untuk tombol CETAK screen SC0141. Migrasi dari
 * legacy {@code FisioterapiController.cetak()} + {@code NoteReport}.
 */
@Service
public class PhysiotherapyNotePrintPdfService {

    private static final DecimalFormat MONEY = new DecimalFormat("#,##0.###");

    private final PhysiotherapyService physiotherapyService;

    public PhysiotherapyNotePrintPdfService(PhysiotherapyService physiotherapyService) {
        this.physiotherapyService = physiotherapyService;
    }

    public byte[] generateNotePdf(Integer noteId) throws Exception {
        WardNoteDetailResponse note = physiotherapyService.getNoteDetail(noteId);
        ...
        document.add(new Paragraph("NOTA TRANSAKSI FISIOTERAPI", new Font(Font.HELVETICA, 11, Font.BOLD)));
