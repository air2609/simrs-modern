...
public class LaboratService {
    ...
    public LaboratMastersResponse getMasters(String username) {
        ...
        List<LaboratPatientTypeResponse> patientTypes = jdbc.query(
                ...
                (rs, row) -> new LaboratPatientTypeResponse(rs.getInt("n_patient_type_id"), rs.getString("v_tpatient"),
                        rs.getString("v_tpatient_desc")));
        List<LaboratEscortResponse> escorts = jdbc.query(
                ...
                (rs, row) -> new LaboratEscortResponse(rs.getInt("n_escort_primary_id"),
                        rs.getString("v_escort_type")));
    ...
    public List<LaboratPanelResponse> getPanels(String tariffClass) {
        ...
        String effectiveTc = (tariffClass != null && !tariffClass.trim().isEmpty())
                ? tariffClass.trim().toUpperCase(Locale.ROOT)
        ...
        switch (key.toUpperCase()) {
            ...
            case "ELEKTROLIT":
                data = new String[][] {
                        ...
                        { "LAB67", "CKMB" }, { "LAB68", "HBDH" }, { "LAB69", "LDH" }, { "LAB70", "CKNAC" },
                        { "LAB71", "TROPONIN I" } };
                break;
            case "MIKROBIOLOGI":
                data = new String[][] {
                        { "LAB54", "GRAM" }, { "LAB55", "NEISSER" }, { "LAB56", "BTA" },
                        { "LAB57", "CANDIDA" }, { "LAB58", "TRICHOMONAS" }, { "LAB59", "KULTUR" },
                        { "LAB60", "SENSITIVITAS TEST" } };
                break;
            case "URINE":
                data = new String[][] {
                        { "LAB91", "RUTIN" }, { "LAB92", "PROTEIN" }, { "LAB93", "REDUKSI" }, { "LAB94", "KETON" },
                        { "LAB95", "SEDIMEN" } };
                break;
            case "FECES":
            ...
            case "NARKOBA":
                data = new String[][] {
                        { "LAB87", "AMPHETAMINE" }, { "LAB88", "BENZODIAZEPINE" }, { "LAB89", "COCCAIN" },
                        { "LAB90", "MORPHINE" } };
                break;
            case "TRANSFUSI":
            ...
            case "LAIN-LAIN":
                data = new String[][] {
                        { "LAB74", "BGA" }, { "LAB75", "PA" }, { "LAB76", "HELICOBACTER" }, { "LAB77", "PAP SMEAR" },
                        { "LAB78", "FNAB" } };
                break;
            default:
                return list;
    ...
    public List<LaboratRegisteredPatientResponse> searchRegisteredPatients(String mrCode, String patientName,
            String address) {
        ...
        return jdbc.query(sql.toString(),
                (rs, row) -> new LaboratRegisteredPatientResponse(
                        ...
                        rs.getString("v_patient_main_addr"), rs.getInt("n_patient_id")),
                params.toArray());
    }
    ...
    public List<LaboratTreatmentOptionResponse> searchTreatments(Integer unitId, String code, String name,
            String tariffClass) {
        StringBuilder sql = new StringBuilder(
                "select t.n_treatment_id, t.v_treatment_code, t.v_treatment_name, "
                        + "coalesce(tf.n_trtfee_fee, 0) as tariff "
        ...
        if (tariffClass != null && !tariffClass.trim().isEmpty()) {
            sql.append(
                    "and tf.n_tclass_id = (select tc.n_tclass_id from ms_treatment_class tc where tc.v_tclass_desc = ?) ");
            ...
            for (LaboratLineItemRequest line : req.getTreatments()) {
                ...
                if (feeId == null) {
                    throw new IllegalArgumentException(
                            "Treatment fee tidak ditemukan untuk treatment ID: " + line.getRefId());
                }
                jdbc.update(
    ...
    public LaboratActionResultResponse validateNote(Integer noteId, String username) {
        ...
        if (updated == 0)
            throw new IllegalStateException("Hanya nota BARU yang bisa divalidasi.");
    ...
    public LaboratActionResultResponse cancelNote(Integer noteId, LaboratCancelRequest req, String username) {
        ...
        if (updated == 0)
            throw new IllegalStateException("Hanya nota BARU yang bisa dibatalkan.");
    ...
    private void createLaboratJournal(Integer noteId, Timestamp now, String username) {
        ...
        String noteNumber = jdbc.queryForObject("select v_note_no from tb_examination where n_exam_id = ?",
                String.class, noteId);
        Integer coaArId = journalService.findCoaIdByGimKey("COA_OUTPATIENT_AR");
        if (coaArId == null)
            coaArId = journalService.findCoaIdByGimKey("COA_INPATIENT_AR");
        Integer coaTreatId = journalService.findCoaIdByGimKey("COA_TREATMENT");
        if (coaTreatId == null)
            coaTreatId = journalService.findCoaIdByGimKey("COA_MISC_TRX");
        ...
        try {
            Map<String, Object> h = jdbc.queryForMap(
                    "select lr.n_lab_rslt_id, lr.v_lab_rslt_code, lr.n_exam_id, "
                            ...
                            + "left join tb_bed_occupancy bo on bo.n_reg_primary_id = e.n_reg_id and bo.d_check_out_time is null "
                            + "left join ms_bed bed on bed.n_bed_id = bo.n_bed_primary_id "
                            + "where lr.n_lab_rslt_id = ?",
                    resultId);
            List<LaboratResultItemResponse> items = getLabResultItems(resultId);
            return new LaboratResultDetailResponse(
                    ((Number) h.get("n_lab_rslt_id")).intValue(), (String) h.get("v_lab_rslt_code"),
                    ((Number) h.get("n_exam_id")).intValue(), (String) h.get("v_note_no"),
    ...
    public LaboratResultSaveResultResponse createLabResult(LaboratResultSaveRequest req, String username) {
        ...
        try {
            existingId = jdbc.queryForObject(
                    "select n_lab_rslt_id from tb_laboratory_result where n_exam_id = ? limit 1", Integer.class,
                    examId);
        } catch (EmptyResultDataAccessException e) {
            /* ok */ }
        if (existingId != null)
        ...
        if (req.getLines() != null) {
            for (LaboratResultLineRequest line : req.getLines()) {
                if (line.getResultDescription() == null || line.getResultDescription().trim().isEmpty())
                    continue;
